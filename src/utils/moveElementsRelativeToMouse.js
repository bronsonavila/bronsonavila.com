...
const moveElementsRelativeToMouse = config => {
  ...
  const { additionalTransformValues, containerSelector, intensityDivisor } = Object.assign({}, defaultConfig, config)
  ...
  containers.forEach(container => {
    ...
    const update = event => {
      ...
      updateTransformStyle((mouse.x / intensityDivisor).toFixed(2), (mouse.y / intensityDivisor).toFixed(2) * -1)
    }
